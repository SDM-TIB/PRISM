...
class StochasticNecessaryExplanationBuilder(NecessaryExplanationBuilder):
    ...
    def build_explanations(self,
                           samples_to_remove: list[tuple[int, int, int]],
                           top_k: int = 10):
        ...
        # samples_with_relevance : ((s, p, o), relevance) = ((int, int, int), float)
        samples_with_relevance = sorted(sample_2_relevance.items(), key=lambda x: x[1], reverse=True)
        
        # all_rules_with_relevance : ([(s, p, o)], relevance) : ([(int, int, int)], float)
        ...
        if best_rule_relevance > self.xsi:
            ...
            return all_rules_with_relevance, self.complexity
        ...
        while cur_rule_length <= samples_number and cur_rule_length <= self.length_cap:
            ...
            if best_rule_relevance > self.xsi:
                print(f"early exit self.xsi: cur_rule_length ({cur_rule_length})") 
                print(f"best_rule_relevance ({best_rule_relevance}) > self.xsi ({self.xsi})")
                break

            cur_rule_length += 1

        return sorted(all_rules_with_relevance, key=lambda x: x[1], reverse=True)[:top_k], self.complexity
    ...
    def extract_rules_with_length(self,
        ...
        while i < len(all_possible_rules_with_preliminary_scores) and not terminate:
            ...
            print("\tObtained relevance: " + str(current_rule_relevance))
    ...
    def _compute_relevance_for_rule(self, nple_to_remove: list[tuple[int, int, int]]):
