...
from src.relevance_engines.engine import ExplanationEngine
...
from src.link_prediction.models.tucker import TuckER
from src.link_prediction.optimization.bce_optimizer import KelpieBCEOptimizer
from src.link_prediction.optimization.multiclass_nll_optimizer import KelpieMultiClassNLLOptimizer
from src.link_prediction.optimization.pairwise_ranking_optimizer import KelpiePairwiseRankingOptimizer
from src.link_prediction.models.model import Model, KelpieModel
...
class PostTrainingEngine(ExplanationEngine):
    ...
    def addition_relevance(self,
                           ...
                           perspective: str,
                           samples_to_add: list,
                           relevant_triples: list[Tuple[str, str, str]] = None):
        """
            ...
            :return: (relevance, original_best_entity_score, original_target_entity_score, original_target_entity_rank, base_pt_best_entity_score, base_pt_target_entity_score, base_pt_target_entity_rank, pt_best_entity_score, pt_target_entity_score, pt_target_entity_rank, execution_time) 
            Where a higher relevance means a more relevant fact. 
        """
        ...
        kelpie_dataset = self._get_kelpie_dataset_for(original_entity_id=original_entity_to_convert, relevant_triples=relevant_triples)
    ...
    def removal_relevance(self,
        ...
        """
            ...
            :param samples_to_remove:   the list of samples containing the perspective entity
                                        that we want to analyze the effect of, if added to the perspective entity
            :param relevant_triples:    for these triples the retraining is executed (only necessary for symbolic learning)
    ...
    def _get_kelpie_dataset_for(self, original_entity_id: int, relevant_triples:list[list[int]]) -> KelpieDataset:
        """
        ...
        if original_entity_id not in self._kelpie_dataset_cache:
            ...
            kelpie_dataset = KelpieDataset(dataset=self.dataset, entity_id=original_entity_id, relevant_triples=relevant_triples)
    ...
    def base_post_training_results_for(self,
        ...
        if not original_sample_to_predict in self._base_pt_model_results:
            ...
            if self.verbose: print("\t\tRunning base post-training on entity " + original_entity_name + " with no additions")
    ...
    def extract_detailed_performances_on_sample(self,
        ...
        all_scores = model.all_scores(arr).detach().cpu().numpy()[0]
