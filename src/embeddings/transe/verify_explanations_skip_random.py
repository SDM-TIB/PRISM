...
filename = output_prefix + "__verify_skip_random_py.txt"
print(filename)
filename2 = output_prefix + f"_{dataset}_output_end_to_end_skipping_random_facts.txt"
filename3 = output_prefix + f"_{dataset}_output_end_to_end_skipping_random_facts.txt"

with open(filename, "w", encoding="utf8" ) as execution_log:
    execution_log.write(f"verify_explanations_skip_random.py at {timestamp}\n")
    execution_log.write(str(args)+"\n")
    execution_log.write(f"kernel: {kernel}, seed: {seed}, output: {filename3} or {filename2}\n")

with open("output.txt", "r", encoding="utf8" ) as input_file:     #TODO: add output path and pick latest file or hand over file
...
if args.mode == "sufficient":
    ...
    print(filename2)
    with open(filename2, "w", encoding="utf8" ) as outfile:
    ...
    print(filename3)
    with open(filename3, "w", encoding="utf8" ) as outfile:
        outfile.writelines(output_lines)
