...
# ✅ ADDED: Import the correct save functions
from src.model.expath_output_utils import save_expath_explanation_necessary, save_expath_explanation_sufficient
...
def explain(
    ...
    if baseline is None:
        ...
        kelpie = Kelpie(model=model, dataset=dataset, hyperparameters=hyperparameters, prefilter_type=prefilter,
         ...
         kelpie = eXpath(model=model, dataset=dataset, hyperparameters=hyperparameters, output_prefix=output_prefix, 
    ...
    # ✅ ADDED: Delete old explanation file to start fresh
    filename_original_output = os.path.join(EXPLAIN_PATH, f"{model_file[:-3]}_explanation.csv")
    if os.path.exists(filename_original_output):
        os.remove(filename_original_output)
        print(f"Deleted old explanation file: {filename_original_output}")
    ...
    for i, fact in enumerate(testing_facts):
        ...
        if mode == "sufficient":
            ...
            # ✅ ADDED: Save in correct format for verification
            save_expath_explanation_sufficient(
                sample_to_explain=sample_to_explain,
                explanations_with_relevance=rule_samples_with_relevance,
                entities_to_convert=entities_to_convert,
                dataset=dataset,
                output_file=filename_original_output
            )
            
            # Keep old format for logging output
            ...
            # ✅ ADDED: Save in correct format for verification
            save_expath_explanation_necessary(
                sample_to_explain=sample_to_explain,
                rules_with_relevance=rule_samples_with_relevance,
                dataset=dataset,
                output_file=filename_original_output
            )
    ...
    try:
        ...
        # ✅ REMOVED: This was overwriting the correct format with the bad format
        # The explanation file is already written correctly by save_expath_explanation_* calls above
        # filename_original_output = os.path.join(EXPLAIN_PATH, f"{model_file[:-3]}_explanation.csv")
