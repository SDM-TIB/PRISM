...
class KelpieDataset(Dataset):
    ...
    def __init__(self,
                 dataset: Dataset,
                 entity_id: int,
                 relevant_triples:list[tuple[int, int, int]] = None):
        ...
        # if symbolic: use extended train samples
        if relevant_triples != None:
            self.original_train_samples = self._extract_samples_with_list(dataset.train_samples, relevant_triples)
            self.original_valid_samples = self._extract_samples_with_list(dataset.valid_samples, relevant_triples)
            self.original_test_samples = self._extract_samples_with_list(dataset.test_samples, relevant_triples)
                                
            #TODO: maybe replace not central entity, but enities along the path
    ...
    def add_training_samples(self, samples_to_add: numpy.array):
        ...
        # extended retraining with sample chains does not work with this assertian
        #for sample in samples_to_add:
        #    assert self.original_entity_id == sample[0] or self.original_entity_id == sample[2], "The samples to remove must still feature the original entity id"
    ...
    def remove_training_samples(self, samples_to_remove: numpy.array):
        ...
        #for sample in samples_to_remove:       #TODO: check if this influences original code...
        #    assert self.original_entity_id == sample[0] or self.original_entity_id == sample[2], "The samples to remove must still feature the original entity id" 
    ...
    @staticmethod
    def _extract_samples_with_entity(samples:list[tuple[int, int, int]], entity_id:int)->list:
        return samples[numpy.where(numpy.logical_or(samples[:, 0] == entity_id, samples[:, 2] == entity_id))]
    
    def _extract_samples_with_list(self, samples:list[list[int, int, int]], triple_list:list[tuple[int, int, int]])->list[tuple[int, int, int]]:
        return_samples = []
        for sample in samples:
            for triple in triple_list:
                if tuple(sample) == triple:
                    return_samples.append(triple)
