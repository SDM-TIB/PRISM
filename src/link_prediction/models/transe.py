...
from src.link_prediction.models.model import Model, DIMENSION, KelpieModel
...
class TransE(Model):
    ...
    def score_embeddings(self,
        ...
        """
            ...
            :return: a numpy array containing the scores computed for the passed triples of embeddings. Low value means little plausibility, high value means high plausibility
        """
        ...
        # mLie: changed scoreing function from "(head_embeddings + rel_embeddings - tail_embeddings).norm(p=1, dim=1)" to the current version with -, so the score is better when rank is high and lower when rank is decreasing. 
    ...
    def all_scores(self, samples: np.array):
        ...
        head_embeddings = self.entity_embeddings[samples[:, 0].tolist()]     # list of entity embeddings for the heads of the facts
        rel_embeddings = self.relation_embeddings[samples[:, 1].tolist()]    # list of relation embeddings for the relations of the fats
    ...
    def predict_tails(self, samples: np.array) -> Tuple[Any, Any, Any]:
        ...
        with torch.no_grad():
            ...
            targets = torch.zeros(size=(len(samples), 1)).cuda()  if torch.cuda.is_available() else  torch.zeros(size=(len(samples), 1))
