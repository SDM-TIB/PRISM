...
from src.model.kelpie_dataset import KelpieDataset
from src.link_prediction.models.model import Model, DIMENSION, INPUT_DROPOUT, FEATURE_MAP_DROPOUT, HIDDEN_DROPOUT, HIDDEN_LAYER_SIZE, KelpieModel
...
class ConvE(Model):
    ...
    def __init__(self,
        ...
        self.input_dropout = torch.nn.Dropout(self.input_dropout_rate).cuda() if torch.cuda.is_available() else torch.nn.Dropout(self.input_dropout_rate)
        self.feature_map_dropout = torch.nn.Dropout2d(self.feature_map_dropout_rate).cuda() if torch.cuda.is_available() else torch.nn.Dropout2d(self.feature_map_dropout_rate)
        self.hidden_dropout = torch.nn.Dropout(self.hidden_dropout_rate).cuda() if torch.cuda.is_available() else torch.nn.Dropout(self.hidden_dropout_rate)
        self.batch_norm_1 = torch.nn.BatchNorm2d(1).cuda() if torch.cuda.is_available() else torch.nn.BatchNorm2d(1)
        self.batch_norm_2 = torch.nn.BatchNorm2d(self.num_conv_filters).cuda() if torch.cuda.is_available() else torch.nn.BatchNorm2d(self.num_conv_filters)
        self.batch_norm_3 = torch.nn.BatchNorm1d(self.dimension).cuda() if torch.cuda.is_available() else torch.nn.BatchNorm1d(self.dimension)
        self.convolutional_layer = torch.nn.Conv2d(1, self.num_conv_filters, self.conv_kernel_shape, 1, 0, bias=True).cuda() if torch.cuda.is_available() else torch.nn.Conv2d(1, self.num_conv_filters, self.conv_kernel_shape, 1, 0, bias=True)
        #self.register_parameter('b', Parameter(torch.zeros(self.num_entities)))     # ?
        self.hidden_layer = torch.nn.Linear(self.hidden_layer_size, self.dimension).cuda() if torch.cuda.is_available() else torch.nn.Linear(self.hidden_layer_size, self.dimension)
        ...
        if init_random:
            self.entity_embeddings = Parameter(torch.rand(self.num_entities, self.dimension).cuda(), requires_grad=True)  if torch.cuda.is_available() else Parameter(torch.rand(self.num_entities, self.dimension), requires_grad=True)
            self.relation_embeddings = Parameter(torch.rand(self.num_relations, self.dimension).cuda(), requires_grad=True) if torch.cuda.is_available() else Parameter(torch.rand(self.num_relations, self.dimension), requires_grad=True)
    ...
    def predict_tails(self, samples: np.array) -> Tuple[Any, Any, Any]:
        ...
        for i in range(0, samples.shape[0], batch_size):
            ...
            if torch.cuda.is_available():
                ...
                tail_indexes = torch.tensor(batch[:, 2])  # tails of all passed samples
...
class KelpieConvE(KelpieModel, ConvE):
    def __init__(
        ...
        self.kelpie_entity_embedding = Parameter(init_tensor.cuda(), requires_grad=True) if torch.cuda.is_available() else Parameter(init_tensor, requires_grad=True)
