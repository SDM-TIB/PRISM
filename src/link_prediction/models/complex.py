...
from src.link_prediction.models.model import Model, KelpieModel, DIMENSION, INIT_SCALE
...
class ComplEx(Model):
    ...
    def __init__(self,
        ...
        if init_random:
            if torch.cuda.is_available():
                ...
                self.entity_embeddings = Parameter(torch.rand(self.num_entities, self.dimension), requires_grad=True)
                self.relation_embeddings = Parameter(torch.rand(self.num_relations, self.dimension), requires_grad=True)
    ...
    def predict_samples(self, samples: np.array) -> Tuple[Any, Any, Any]:
        ...
        assert np.all(direct_samples[:, 1] < self.dataset.num_direct_relations), "make sure that all the passed samples are 'direct' samples"
    ...
    def predict_tails(self, samples: np.array) -> Tuple[Any, Any, Any]:
        ...
        with torch.no_grad():
            ...
            if torch.cuda.is_available():
                ...
                targets = torch.zeros(size=(len(samples), 1))
...
class KelpieComplEx(KelpieModel, ComplEx):
    def __init__(
        ...
        if torch.cuda.is_available():
            ...
            self.kelpie_entity_embedding = Parameter(init_tensor, requires_grad=True)
    ...
    def predict_samples(self,
        ...
        assert (samples[:, 1] < self.dataset.num_direct_relations).all(), "make sure that all the passed samples are 'direct' samples"
        ...
        assert np.isin(forbidden_entity_id, direct_samples[:][0, 2]) == False, "if we are in original_mode, make sure that the kelpie entity is not featured in the samples to predict. otherwise, make sure that the original entity is not featured in the samples to predict"
    ...
    def predict_sample(self,
        ...
        assert sample[1] < self.dataset.num_direct_relations #TODO: give all assert an error message
