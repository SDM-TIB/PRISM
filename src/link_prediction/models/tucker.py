...
class TuckER(Model):
    ...
    def __init__(self,
        ...
        if init_random:
            self.entity_embeddings = Parameter(torch.empty(self.num_entities, self.entity_dimension).cuda(), requires_grad=True) if torch.cuda.is_available() else Parameter(torch.empty(self.num_entities, self.entity_dimension), requires_grad=True)
            self.relation_embeddings = Parameter(torch.empty(self.num_relations, self.relation_dimension).cuda(), requires_grad=True) if torch.cuda.is_available() else Parameter(torch.empty(self.num_relations, self.relation_dimension), requires_grad=True)
            self.core_tensor = Parameter(torch.tensor(np.random.uniform(-1, 1, (self.relation_dimension, self.entity_dimension, self.entity_dimension)), dtype=torch.float).cuda(), requires_grad=True) if torch.cuda.is_available() else Parameter(torch.tensor(np.random.uniform(-1, 1, (self.relation_dimension, self.entity_dimension, self.entity_dimension)), dtype=torch.float), requires_grad=True)
    ...
    def all_scores(self, samples: np.array):
        ...
        head_indexes, relation_indexes = samples[:, 0].tolist(), samples[:, 1].tolist()
    ...
    def predict_tails(self, samples):
        ...
        for i in range(0, samples.shape[0], batch_size):
            ...
            tail_indexes = torch.tensor(batch[:, 2]).cuda() if torch.cuda.is_available() else torch.tensor(batch[:, 2])  # tails of all passed samples
