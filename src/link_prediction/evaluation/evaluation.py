...
from config import LOGGING_PATH, RANKS_PATH

class Evaluator:
    def __init__(self, model: Model, output_prefix=""):
        self.model = model
    ...
    def _write_output(self, samples, ranks, predictions):
        ...
        data_folder_file = os.path.join(RANKS_PATH, f"{self.model.name}_{self.dataset.name}_filtered_ranks.csv")
        print(data_folder_file)
        with open(data_folder_file, "w", encoding="utf8" ) as output_file:
            output_file.writelines(result_lines)

        filename1 = os.path.join(LOGGING_PATH, self.output_prefix + "_4_filtered_ranks.csv")
        print(filename1)
        with open(filename1, "w", encoding="utf8" ) as output_file:
            output_file.writelines(result_lines)

        filename2 = os.path.join(LOGGING_PATH, self.output_prefix + "_5_filtered_details.csv")
        print(filename2)
        with open(filename2, "w", encoding="utf8" ) as output_file:
