...
from src.link_prediction.regularization.regularizers import L2
from src.link_prediction.models.model import Model, BATCH_SIZE, LEARNING_RATE, EPOCHS, MARGIN, NEGATIVE_SAMPLES_RATIO, \
    REGULARIZER_WEIGHT, KelpieModel
from src.link_prediction.optimization.optimizer import Optimizer
...
class PairwiseRankingOptimizer(Optimizer):
    ...
    def __init__(self,
        ...
        self.loss = torch.nn.MarginRankingLoss(margin=self.margin, reduction="mean").cuda() if torch.cuda.is_available() else torch.nn.MarginRankingLoss(margin=self.margin, reduction="mean")
    ...
    def train(self,
        ...
        for e in range(1, self.epochs+1):
            ...
            if evaluate_every > 0 and valid_samples is not None and e % evaluate_every == 0:
                ...
                if self.verbose: print("\tValidation Hits@1: %f" % h1)
                if self.verbose: print("\tValidation Hits@10: %f" % h10)
                if self.verbose: print("\tValidation Mean Reciprocal Rank': %f" % mrr)
                if self.verbose: print("\tValidation Mean Rank': %f" % mr)
    ...
    def step_on_batch(self, positive_batch, negative_batch):
        ...
        target = torch.tensor([-1], dtype=torch.float).cuda() if torch.cuda.is_available() else torch.tensor([-1], dtype=torch.float)
