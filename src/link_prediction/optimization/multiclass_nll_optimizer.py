...
from src.link_prediction.regularization.regularizers import N3, N2
from src.link_prediction.models.model import Model, OPTIMIZER_NAME, BATCH_SIZE, EPOCHS, LEARNING_RATE, DECAY_1, REGULARIZER_NAME, \
...
class MultiClassNLLOptimizer(Optimizer):
    ...
    def epoch(self,
        ...
        training_samples = torch.from_numpy(training_samples).cuda() if torch.cuda.is_available() else torch.from_numpy(training_samples)
        ...
        with tqdm.tqdm(total=training_samples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            while batch_start < training_samples.shape[0]:
                ...
                batch = actual_samples[batch_start : batch_end].cuda() if torch.cuda.is_available() else actual_samples[batch_start : batch_end]
...
class KelpieMultiClassNLLOptimizer(MultiClassNLLOptimizer):
    ...
    def epoch(self,
        ...
        with tqdm.tqdm(total=training_samples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            batch_start = 0
            while batch_start < training_samples.shape[0]:
                batch = actual_samples[batch_start: batch_start + batch_size].cuda() if torch.cuda.is_available() else actual_samples[batch_start: batch_start + batch_size]
                l = self.step_on_batch(loss, batch)
