...
class TopologyPreFilter(PreFilter):
    ...
    def top_promising_samples_for(self,
                                  ...
                                  perspective:str,
                                  top_k=50)->List[Tuple[int, int, int]]:
        ...
        :return: the sorted list of the k most promising samples. Or an empty list.
        """
        ...
        if self.verbose: print("(topology prefilter) Extracting promising facts for" + self.dataset.printable_sample(sample_to_explain))
        ...
        if start_entity not in self.entity_id_2_train_samples:
            print(f"skipping explanation for triple '{self.dataset.printable_sample(sample_to_explain)}': '{start_entity}' not in training set")
            return []
        
        # find all triples containing entity
        ...
        worker_processes_inputs = [(len(samples_featuring_start_entity),
                                   start_entity, end_entity, samples_featuring_start_entity[i], self.verbose)
    	...
    	# somehow sorted list of triples containing entity
    ...
    def analyze_sample(self, input_data):
        ...
        while not terminate:
            ...
            if terminate is not True: # identical to "if terminate is False"
