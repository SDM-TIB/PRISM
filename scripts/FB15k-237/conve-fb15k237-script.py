...
def run_pipeline(config, builder):
    ...
    exp_id = config.get("exp_id", "_convE100")  # must start with _
    message = config.get("message", f"[experiments] conve {builder}")
...
def main():
    ...
    try:
        with open('input_fb15k237_conve.json', 'r') as file:
            config = json.load(file)
    except FileNotFoundError:
        print("Error: input_fb15k237_conve.json file not found.")
        return
    except json.JSONDecodeError:
        print("Error: input_fb15k237_conve.json contains invalid JSON.")
        return
