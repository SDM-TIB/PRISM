...
def run_pipeline(config, builder):
    ...
    predictions = config.get("predictions", f"{embedding_model.lower()}_db100k_random.csv")
    ...
    dimension = config.get("dimension", 200)
    ...
    exp_id = config.get("exp_id", "_transE_100")  # must start with _
    message = config.get("message", f"[experiments] transe {builder}")
...
def main():
    ...
    try:
        with open('input_DB100K_TransE.json', 'r') as file:
            config = json.load(file)
    except FileNotFoundError:
        print("Error: input_db100k.json file not found.")
        return
    except json.JSONDecodeError:
        print("Error: input_db100k.json contains invalid JSON.")
        return
