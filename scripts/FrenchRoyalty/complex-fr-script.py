...
def run_pipeline(config, builder):
    ...
    dimension = config.get("dimension", 1000)  # Note: gets doubled in ComplEx
...
def main():
    ...
    try:
        with open('input_complex_fr.json', 'r') as file:
            config = json.load(file)
    except FileNotFoundError:
        print("Error: input_complex_fr.json file not found.")
        return
    except json.JSONDecodeError:
        print("Error: input_complex_fr.json contains invalid JSON.")
        return
